...
class WatchlistsManager:
        ...
        try:
            ...
            async with conn.transaction():
                table_exists = await conn.fetchval(
                    "SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'watchlists');"
                )
                if table_exists:
                    qty_column_exists = await conn.fetchval(
                        "SELECT EXISTS (SELECT FROM information_schema.columns WHERE table_name = 'watchlists' AND column_name = 'quantity');"
                    )
    ...
    async def _db_operation_with_retry(
        self, operation_type: str, query: str, *params, retries: int = 3
    ):
        ...
        for attempt in range(retries + 1):
            ...
            try:
                ...
                print(f"A non-connection error occurred: {e}")
                raise
            finally:
                if conn and not conn.is_closed():
                    await conn.close()
    ...
    async def _execute_with_retry(self, query: str, *params):
        return await self._db_operation_with_retry("execute", query, *params)

    async def _fetchval_with_retry(self, query: str, *params):
        return await self._db_operation_with_retry("fetchval", query, *params)

    async def _fetch_with_retry(self, query: str, *params):
        return await self._db_operation_with_retry("fetch", query, *params)
    ...
    async def get_watchlists_count(self, user_id: str) -> int:
        try:
            count = await self._fetchval_with_retry(
