...
def retry_on_rate_limit(
    max_retries: int = 3, base_delay: float = 5.0, success_delay: float = 1.5
):
                    ...
                    if (
                        ...
                        if attempt < max_retries - 1:
                            ...
                            wait_time = base_delay * (
                                3**attempt
                            )  # 5s, 15s, 45s progression
                            logger.warning(
                                f"Rate limited on attempt {attempt + 1}, waiting {wait_time}s before retry"
                            )
                            time.sleep(wait_time)
                            continue
                        else:
                            logger.error(
                                f"Max retries ({max_retries}) reached for rate limiting"
                            )
                            raise
                    else:
...
def get_recommendations(ticker: str, limit: int = 5) -> pd.DataFrame | None:
    ...
    try:
        ...
        return df.head(limit) if df is not None else None  # type: ignore
    except Exception as e:
...
def get_upgrades_downgrades(ticker: str, limit: int = 5) -> pd.DataFrame | None:
    ...
    try:
        ...
        return df.sort_index(ascending=False).head(limit) if df is not None else None  # type: ignore
    except Exception as e:
...
def get_financial_statements(
    ...
    try:
        ...
        logger.error(
            f"Error retrieving {frequency} {statement_type} statement for {ticker}: {e}"
        )
        return None
...
def get_institutional_holders(
    ...
    try:
        ...
        return (
            inst.head(top_n) if inst is not None else None,  # type: ignore
            fund.head(top_n) if fund is not None else None,  # type: ignore
        )
    except Exception as e:
...
def get_options_chain(
    ticker: str, expiry: str | None = None, option_type: Literal["C", "P"] | None = None
...
def get_filtered_options(
    ...
    try:
        ...
        return (
            df.sort_values(["openInterest", "volume"], ascending=[False, False]),
            None,
        )
