            ...
            def parse_timestamp(ts_value):
                if isinstance(ts_value, (int, float)):
                    ...
                    try:
                        ...
                        dt_obj = datetime.fromisoformat(ts_value).astimezone(
                            timezone.utc
                        )
                ...
                logger.warning(
                    f"Unexpected timestamp type: {type(ts_value)}, value: {ts_value}"
                )
                return None

            # Process current fear_and_greed timestamp
            if (
                "fear_and_greed" in data
                and isinstance(data["fear_and_greed"], dict)
                and "timestamp" in data["fear_and_greed"]
            ):
                ...
                if parsed_ts is not None:
                    ...
                    logger.error(
                        f"Failed to parse current timestamp: {original_ts}. Removing field."
                    )
                    del data["fear_and_greed"]["timestamp"]

            # Process historical timestamps
            if (
                "fear_and_greed_historical" in data
                ...
                historical_entries = data["fear_and_greed_historical"][
                    "data"
                ...
                for entry in historical_entries:
                    ...
                    if isinstance(entry, dict) and "x" in entry and "y" in entry:
                        ...
                        if parsed_ts is not None:
                            ...
                            processed_entry["timestamp"] = (
                                parsed_ts  # Add a standard 'timestamp' key
                            )
                            processed_entry["score"] = entry[
                                "y"
                            ]  # Add a standard 'score' key
                            ...
                            logger.warning(
                                f"Failed to parse historical timestamp: {original_ts}. Skipping entry."
                            )
                    else:
                        logger.warning(
                            f"Historical entry has unexpected structure or missing keys 'x'/'y': {entry}. Skipping entry."
                        )
                ...
                logger.warning(
                    "Historical F&G data found but has unexpected structure."
                )
                # Decide how to handle this: remove it, leave it as is, etc.
                data["fear_and_greed_historical"] = (
                    []
                )  # Default to empty list if structure is wrong
